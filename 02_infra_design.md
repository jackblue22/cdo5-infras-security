...
## 2.1 Baseline Terraform đã validate cho Milestone 1

Milestone 1 hiện tập trung vào **AWS infrastructure + security baseline**, chưa deploy toàn bộ workload Kubernetes. Terraform trong `temp/aiops/terraform` đã được validate bằng apply thật trên AWS và đang bám theo thiết kế sau:

Region mặc định của CDO-05 cho milestone này là `us-east-1`, để khớp với AIO-01 image handoff, Bedrock/AIO contract hiện tại và tránh vấn đề private node phải pull cross-region ECR image.

| Hạng mục | Baseline đã chạy được | Ghi chú production |
|---|---|---|
| EKS control plane | EKS `1.30`, private subnets, public + private API endpoint cho dev | Production nên giới hạn `cluster_endpoint_public_access_cidrs` hoặc tắt public endpoint. |
| EKS worker nodes | `2 x m7i-flex.large`, `AL2023_x86_64_STANDARD` | Chọn x86_64 để khớp AI image hiện tại và đủ RAM cho EKS add-ons + AI Engine + worker + observability demo. Fallback `t3.micro`/`t3.small` chỉ dùng khi cần validate infra rẻ nhất, không nên dùng cho workload demo đầy đủ. |
| EKS managed add-ons | `vpc-cni`, `kube-proxy`, `coredns`, `aws-ebs-csi-driver` | `vpc-cni`/`kube-proxy` tạo trước node group; `coredns`/`aws-ebs-csi-driver` tạo sau node group. EBS CSI dùng IRSA riêng cho `kube-system/ebs-csi-controller-sa`. |
| Private AWS access | VPC endpoints cho SQS, ECR API/DKR, EC2, STS, Secrets Manager, KMS, CloudWatch Logs, S3, DynamoDB | Giảm phụ thuộc NAT Gateway và giữ pod trong private subnet. Với default `us-east-1`, EKS private nodes pull AIO ECR image cùng region qua endpoint nếu cross-account permission còn hiệu lực. |
| Alert reliability | SQS FIFO + FIFO DLQ, Lambda ingest, DynamoDB state, S3 audit/evidence | SQS chỉ chứa alert event; raw telemetry vẫn ở Prometheus/Loki/CloudWatch. |
| Workload image handoff | AI Engine image do AIO-01 cung cấp: `589077667575.dkr.ecr.us-east-1.amazonaws.com/tf1-ai-triage-engine:v1.0.0` | Khi deploy workload, nên pin digest `sha256:ed9d9ca831aa70865e175a611359610c66be5cb56fd33b0487ac687fc4b14f70`. Nếu CDO đổi region khỏi `us-east-1`, cần replicate/copy image sang ECR cùng region hoặc bật NAT có kiểm soát. |

Điểm cần hiểu: baseline này chứng minh hạ tầng AWS đã đủ để gắn workload layer vào sau. Terraform đã từng apply một nhịp từ empty state, verify EKS/node/add-ons healthy, sau đó destroy sạch để tránh cost. Các manifest/Helm/GitOps cho namespace `app`, `aiops`, `ai-engine`, `observability`, NetworkPolicy và deployment của AI Engine/Worker thuộc bước tiếp theo.
