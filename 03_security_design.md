...
**Nguồn căn cứ:** `01_requirements_analysis.md`, `02_infra_design.md`
...
Region mặc định của CDO-05 là `us-east-1`. Điều này quan trọng về security/network vì AI Engine image của AIO-01 cũng nằm trong ECR `us-east-1`; EKS private nodes có thể pull image cùng region qua ECR/VPC endpoints khi cross-account pull permission hợp lệ, không cần mở NAT chỉ để kéo image cross-region.
...
| `tf1-ebs-csi-driver-role` | EKS managed add-on qua IRSA | AWS managed policy `AmazonEBSCSIDriverPolicy` cho service account `kube-system/ebs-csi-controller-sa`, dùng để create/attach/detach EBS volume cho PVC. | Không cấp cho app/worker/AI pod; không dùng role này để đọc SQS, DynamoDB, S3 hoặc Secrets Manager. |
...
AI Engine runtime image do AIO-01 bàn giao cho CDO deploy lên EKS:

```text
image: 589077667575.dkr.ecr.us-east-1.amazonaws.com/tf1-ai-triage-engine:v1.0.0
digest: 589077667575.dkr.ecr.us-east-1.amazonaws.com/tf1-ai-triage-engine@sha256:ed9d9ca831aa70865e175a611359610c66be5cb56fd33b0487ac687fc4b14f70
containerPort: 8080
health: GET /healthz, GET /readyz, GET /metrics
api: POST /v1/triage
```

Security requirement khi triển khai workload:

- Với default region `us-east-1`, ưu tiên dùng image AIO-01 trực tiếp bằng pinned digest vì EKS và ECR cùng region.
- EKS node/pod phải có quyền pull cross-account ECR đúng như AIO-01 đã cấp; không đưa ECR credential vào Kubernetes Secret thủ công.
- Nếu CDO deploy EKS ở region khác `us-east-1`, cần replicate/copy image sang ECR cùng region hoặc bật NAT/egress có kiểm soát; không giả định VPC endpoint ở region khác có thể pull image `us-east-1`.
- AI Engine service vẫn chỉ expose nội bộ trong namespace `ai-engine`, không expose public internet.
- Digest được ưu tiên cho prod-like deployment; tag `v1.0.0` chỉ dùng cho demo nhanh hoặc khi đã chấp nhận rủi ro mutable tag.
...
| EC2 endpoint | EKS nodes / VPC CNI | Cho phép private nodes và VPC CNI gọi EC2 API mà không cần NAT Gateway. |
