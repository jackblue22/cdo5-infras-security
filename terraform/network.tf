...
locals {
  interface_endpoint_services = concat(
    ...
    var.enable_bedrock_endpoint ? ["com.amazonaws.${var.aws_region}.bedrock-runtime"] : []
  )
}
...
resource "aws_vpc_endpoint" "interface" {
  ...
  tags = {
    Name = "${local.name_prefix}-${replace(replace(each.value, "com.amazonaws.${var.aws_region}.", ""), ".", "-")}-vpce"
  }
}
