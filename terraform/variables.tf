...
variable "node_instance_types" {
  description = "Managed node group instance types. m7i-flex.large is the recommended x86_64 demo node when the account/free-plan allows it; use t3.micro/t3.small only for cheapest infra-only validation."
  type        = list(string)
  default     = ["m7i-flex.large"]
}

variable "node_ami_type" {
  description = "EKS managed node group AMI type. AL2023 is the current production baseline for new x86_64 EKS nodes; AWS resolves the latest patched AMI automatically."
  type        = string
...
variable "enable_ebs_csi_addon" {
  description = "Install the EBS CSI managed add-on. Enable for m7i-flex/t3.small+ demo clusters or real PVC workloads; disable only for tiny t3.micro infra-only validation."
  type        = bool
  default     = true
}
...
variable "enable_ingest_lambda_function_url" {
  description = "Create Lambda Function URL for alert ingestion. Prefer AWS_IAM auth; use NONE only for demo with HMAC validation."
  type        = bool
  default     = true
}
